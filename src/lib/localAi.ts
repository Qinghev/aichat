...
export const generateMoment = (state: AppState, character: Character): MomentPost => {
  const momentTemplates: Record<string, string[]> = {
    c_linxia: [
      "买花没有特别的理由。只是回家的路上，想让桌面多一点今天。",
      "风把窗帘吹起来的时候，房间突然像换了一种心情。",
      "路过一家小店，门口很安静。站了一会儿才发现，原来自己也不赶时间。"
    ],
    c_zhouyu: [
      "今天删掉了三件不重要的事。空出来的时间，才是计划真正开始的地方。",
      "散步时想明白一件事：有些问题不是要解决，是要停止反复证明。",
      "把一个混乱的问题写下来，它就从一团雾变成了几行字。几行字就能处理。"
    ],
    c_atang: [
      "今日份结论：衣服可以低调，人不必。",
      "吃到一家意外不错的小店。先记一笔，下次抓个人一起去。",
      "今天的搭配没什么道理，但镜子说可以，那就可以。"
    ],
    c_muxi: [
      "傍晚的光只停了一会儿，但已经够把今天照得温柔一点。",
      "书翻到一半，下起了雨。剩下的内容忽然不急着读完了。",
      "路上看到天色变暗，突然觉得慢一点也没关系。"
    ]
  };
  const templates = momentTemplates[character.id] || [
